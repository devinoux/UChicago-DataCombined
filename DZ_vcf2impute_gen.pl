...
sub PrintGenFile($$) {
  ...
  while (defined ($line = <VCF>)) {
   ...
   # if ($line_info[$filter_tag_idx] ne "PASS" && $line_info[$filter_tag_idx] ne "." && $line_info[$filter_tag_idx] ne "PASS;GENOTYPED") {
   #   ++$n_filtered_snps;
   #   next;
   # }
	...
	for my $k (0..2) {
	 # $gt_like_info[$k] /= $tot;
	  print GEN " ".sprintf("%.3f",$gt_like_info[$k]);
	}
      }
